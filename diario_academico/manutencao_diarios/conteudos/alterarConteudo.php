...
if (mysqli_num_rows($sql) > 0) {
  ...
  while($ln = mysqli_fetch_assoc($sql)){
    if($id_etapa==$ln['id_etapas'] && strcmp($conteudo,$ln['conteudos'])==0 && strcmp($data,$ln['datas'])==0  ){
      $verif=false;
      echo "<script>alert('Não é possível alterar esse conteúdo');</script>";
...
if($verif==true){
  mysqli_query($con,"UPDATE conteudos SET id_etapas='$id_etapa',id_disciplinas='$id_disciplinas', conteudos='$conteudo',datas='$data' WHERE id='$id' AND id_disciplinas='$id_disciplinas'");
  echo "<script>location.href='conteudo.php'</script>";
}
