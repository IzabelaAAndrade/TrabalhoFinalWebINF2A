...
if(empty($faltando)){
  ...
  if ($existe === true) {
    //executa a inserção
    $insert = "UPDATE diario
        SET `faltas` = $faltas, `nota` = $nota
        WHERE (`id_conteudos`, `id_matriculas`, `id_atividades`) = ($id_conteudos, $id_matriculas, $id_atividades)";
    $result_upd = mysqli_query($cnx, $insert);
