...
$resultado_alunos = mysqli_query($conexao,$query) or die("<div>Erro de conexão!</div>");
...
while ($row_alunos = mysqli_fetch_array($resultado_alunos)){
    ...
    $resultado_matriculas = mysqli_query($conexao,$query) or die("<div>Erro de conexão!</div>");
    ...
    while($row_matriculas = mysqli_fetch_array($resultado_matriculas)){
        ...
        $resultado_diario = mysqli_query($conexao,$query) or die("<div>Erro de conexão!</div>");
    ...
    if($num_matriculas==0){
        echo "<td>Nada consta.</td>
            <td>--</td>";
    }
    else if($num_reprovacoes>0){
