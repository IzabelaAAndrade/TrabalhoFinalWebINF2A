  ...
  if(isset($_POST['busca'])){
    ...
    if($queryResultado > 0){
      ...
      while($row = mysqli_fetch_array($resultado)) {
        $aspas = '"';
        $return .= "<tr><td id='campoId".$row['id']."'>".$row['id']."</td><td id='campoNome".$row['id']."'>".$row['nome']."</td><td>".$row['id_cursos']."</td><td><button id='btnEditar".$row['id']."' onclick='getIdTurma(".$row['id'].");' class='btnAlterar btn btn-info btn-lg' data-toggle='modal' data-target='#alterarTurmaModal'>Editar</button></td><td><button onclick='getDadosDeletar(".$row['id'].",".$aspas.$row['nome'].$aspas.");' id='btnDeletar".$row['id']."' class='btnDeletar btn btn-info btn-lg' data-toggle='modal' data-target='#deletarTurmaModal'>Deletar</button></td></tr>";
      }
      $return .= "</tbody></table>";
    } else{
